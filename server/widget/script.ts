...
(function(){
  ...
  function render(){
    ...
    var lastHtml = showLastCleaned ? '<div class="tqt-hosted-field"><label class="tqt-hosted-field-label" for="tqt-hosted-last-cleaned">When was the yard last cleaned?</label><select id="tqt-hosted-last-cleaned" name="last_time_yard_was_thoroughly_cleaned" class="tqt-hosted-select">'+lastTimes.map(function(o){ return '<option value="'+esc(o.value)+'"'+(String(o.value)===String(state.selection.last_time_yard_was_thoroughly_cleaned)?' selected':'')+'>'+esc(o.label)+'</option>'; }).join('')+'</select></div>' : '';
